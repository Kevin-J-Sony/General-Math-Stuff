...
typedef struct {
	// compute the index (i, j) using row-major order
	void* BLOCK;
	int row_size, col_size;

	// there needs to be some indication to determine whether the block contains more blocks
	// or if it's the base level. If it's at base, then what will occur is regular matrix multiplication.
	// Otherwise, recursively apply the multiplication
	int level;
} block;

void block_multiply(block* block1, block* block2, block* block3) {	
	if (block1 == NULL || block2 == NULL) {
		fprintf(stderr, "One of the blocks is NULL\n");
		return;
	}
	if (block1->level != block2->level) {
		fprintf(stderr, "The blocks aren't on the same level.\n\n");
		return;
	}
	int f_row
